...
const secretHashSha256 = '0x' + crypto.createHash('sha256').update(secret).digest('hex');
...
contract('EtomicSwap', function(accounts) {
    ...
    it('should allow to send ETH payment sha256', async function () {
        const lockTime = await currentEvmTime() + 1000;
        const params = [
            id,
            accounts[1],
        ...
        await this.swap.ethPaymentSha256(...params, { value: web3.utils.toWei('1') }).should.be.fulfilled;
    ...
    it('should allow to send ERC20 payment sha256', async function () {
        const lockTime = await currentEvmTime() + 1000;
        const params = [
            id,
            web3.utils.toWei('1'),
        ...
        await this.token.approve(this.swap.address, web3.utils.toWei('1'));
        await this.swap.erc20PaymentSha256(...params).should.be.fulfilled;
    ...
    it('should allow sender to refund ETH payment after locktime sha256', async function () {
        const lockTime = await currentEvmTime() + 1000;
        const params = [
            id,
            accounts[1],
        ...
        // not allow to refund if payment was not sent
        await this.swap.senderRefundSha256(id, web3.utils.toWei('1'), secretHashSha256, zeroAddr, accounts[1]).should.be.rejectedWith(EVMThrow);
        ...
        // not allow to call refund from non-sender address
        await this.swap.senderRefundSha256(id, web3.utils.toWei('1'), secretHashSha256, zeroAddr, accounts[1], { from: accounts[1] }).should.be.rejectedWith(EVMThrow);

        // not allow to refund invalid amount
        await this.swap.senderRefundSha256(id, web3.utils.toWei('2'), secretHashSha256, zeroAddr, accounts[1]).should.be.rejectedWith(EVMThrow);

        // success refund
        const balanceBefore = web3.utils.toBN(await web3.eth.getBalance(accounts[0]));
        const gasPrice = web3.utils.toWei('100', 'gwei');

        const tx = await this.swap.senderRefundSha256(id, web3.utils.toWei('1'), secretHashSha256, zeroAddr, accounts[1], { gasPrice }).should.be.fulfilled;
        const balanceAfter = web3.utils.toBN(await web3.eth.getBalance(accounts[0]));
    ...
    it('should allow sender to refund ERC20 payment after locktime sha256', async function () {
        const lockTime = await currentEvmTime() + 1000;
        const params = [
            id,
            web3.utils.toWei('1'),
        ...
        // not allow to refund if payment was not sent
        await this.swap.senderRefundSha256(id, web3.utils.toWei('1'), secretHashSha256, this.token.address, accounts[1]).should.be.rejectedWith(EVMThrow);
        ...
        // not allow to call refund from non-sender address
        await this.swap.senderRefundSha256(id, web3.utils.toWei('1'), secretHashSha256, this.token.address, accounts[1], { from: accounts[1] }).should.be.rejectedWith(EVMThrow);

        // not allow to refund invalid amount
        await this.swap.senderRefundSha256(id, web3.utils.toWei('2'), secretHashSha256, this.token.address, accounts[1]).should.be.rejectedWith(EVMThrow);

        // success refund
        const balanceBefore = web3.utils.toBN(await this.token.balanceOf(accounts[0]));

        await this.swap.senderRefundSha256(id, web3.utils.toWei('1'), secretHashSha256, this.token.address, accounts[1]).should.be.fulfilled;
    ...
    it('should allow receiver to spend ETH payment by revealing a secret sha256', async function () {
        const lockTime = await currentEvmTime() + 1000;
        const params = [
            id,
            accounts[1],
    ...
    it('should allow receiver to spend ERC20 payment by revealing a secret sha256', async function () {
        const lockTime = await currentEvmTime() + 1000;
        const params = [
            id,
            web3.utils.toWei('1'),
    ...
    it('should allow receiver to spend ETH payment by revealing a secret even after locktime sha256', async function () {
        const lockTime = await currentEvmTime() + 1000;
        const params = [
            id,
            accounts[1],
    ...
    it('should allow receiver to spend ERC20 payment by revealing a secret even after locktime sha256', async function () {
        const lockTime = await currentEvmTime() + 1000;
        const params = [
            id,
            web3.utils.toWei('1'),
